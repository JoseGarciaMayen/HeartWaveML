...
from src.training.train_tcn import _tcn_block
...
class TunerTCN(TunerBase):
    def __init__(self):
        cfg = TUNING["tcn"]
        super().__init__("ECG_TCN_tuning", n_trials=cfg["n_trials"])
    ...
    def _build_model(self, filters, kernel_size, num_blocks, dropout, lr):
        inp = Input(shape=(self.window, self.n_features))
        x = inp
        for i in range(num_blocks):
            x = _tcn_block(x, filters, kernel_size, 2**i, dropout)
