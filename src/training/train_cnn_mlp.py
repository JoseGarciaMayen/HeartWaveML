...
class TrainerCNNMLP(TrainerBase):
    ...
    def create_model(self, input_shape_cnn=(187, 1), input_shape_mlp=(36,), num_classes=5):
        ...
        x = Conv1D(
            filters=best_params["filters"] * best_params["filter_multiplier"],
            kernel_size=5,
            activation="relu",
            kernel_regularizer=regularizers.l2(best_params["l2"]),
            padding="same",
        ...
        y = Dense(
            best_params["units_mlp1"],
            activation="relu",
            kernel_regularizer=regularizers.l2(best_params["l2"]),
        )(input_mlp)
        y = Dense(
            best_params["units_mlp2"],
            activation="relu",
            kernel_regularizer=regularizers.l2(best_params["l2"]),
        )(y)
        y = Dense(
            best_params["units_mlp3"],
            activation="relu",
            kernel_regularizer=regularizers.l2(best_params["l2"]),
        )(y)
        ...
        z = Dense(64, activation="relu", kernel_regularizer=regularizers.l2(best_params["l2"]))(
            combined
        )
    ...
    def mlflow_start(self, model, X_train, y_train, X_cv, y_cv, class_weights):
        ...
        with mlflow.start_run(nested=True):
            model.fit(
                [X_train.iloc[:, :187].values.reshape(-1, 187, 1), X_train.iloc[:, 187:].values],
                y_train,
                class_weight=class_weights,
                validation_data=(
                    [X_cv.iloc[:, :187].values.reshape(-1, 187, 1), X_cv.iloc[:, 187:].values],
                    y_cv,
                ),
                epochs=50,
            )
            ...
            val_loss, val_acc = model.evaluate(
                [X_cv.iloc[:, :187].values.reshape(-1, 187, 1), X_cv.iloc[:, 187:].values], y_cv
            )
            mlflow.log_metric("val_accuracy", val_acc)
            mlflow.log_metric("val_loss", val_loss)

            y_cv_logits = model.predict(
                [X_cv.iloc[:, :187].values.reshape(-1, 187, 1), X_cv.iloc[:, 187:].values]
            )
    ...
    def train(self):
        ...
        X_train, y_train, X_cv, y_cv, class_weights = self.load_data(
            "data/processed/feat/mitbih_train_features.csv",
            "data/processed/feat/mitbih_cv_features.csv",
        )
