...
def _tcn_block(x, filters, kernel_size, dilation_rate, dropout):
    prev = x
    y = Conv1D(
...
class TrainerTCN(TrainerBase):
    def __init__(self, model_name="modelTCN", experiment_name="ECG_TCN"):
    ...
    def create_model(self, window, n_features, p):
        ...
        for i in range(p["num_blocks"]):
            x = _tcn_block(x, p["filters"], p["kernel_size"], 2**i, p["dropout"])
    ...
    def run_training(
        ...
        print(
            f"\nTCN CV - F1-N:{val_f1_per[0]:.4f}  F1-S:{val_f1_per[1]:.4f}  "
    ...
    def train(self):
        ...
        notify_telegram(f"TCN trained - val_f1_macro: {val_f1:.4f}")
        return val_f1
